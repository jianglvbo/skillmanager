...
向用户报告：创建了哪些条目（标题 + 分类 + 归属层）、是否更新博主档案、条目间关联方向、可验证判断提示（第二步第 6 条）、产出校验结果、待决策队列处理结果（第一步 1.1 第 5 条）。**批量提炼（帖子积压很多、分多轮推）时每轮汇报末尾跑一次 `node src/scripts/refine-progress.js`**（只读，出一根「已处理/总量」进度条）；不知下一批挑谁时跑 `node src/scripts/refine-progress-by-blogger.js`（每位博主一根条、按剩余倒序）。两条都只出「条 + (已/总)」，不带其它文字。单条/少量提炼不必跑。
