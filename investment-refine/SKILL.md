...
向用户报告：创建了哪些条目（标题 + 分类 + 归属层）、是否更新博主档案、条目间关联方向、可验证判断提示（第二步第 6 条）、产出校验结果、待决策队列处理结果（第一步 1.1 第 5 条）。**批量提炼（帖子积压很多、分多轮推）时每轮汇报末尾跑一次 `node src/scripts/refine-progress.js`**（读 `src/config.json` 的 mysql，只读统计 `post_history`）贴出 ASCII 进度：总进度条 =「已处理/总量」，另附**本轮质量条** = 自上次运行增量里「已落言论 vs 判无价值」占比（基线存 `src/scripts/.refine-progress.state.json`，已 gitignore），用来看这一批提炼得怎么样。**接不下手 / 纠结下一批挑谁时再跑 `node src/scripts/refine-progress-by-blogger.js`**（博主维度、按「待提炼」倒序，最痛的博主置顶，末尾合计应等于总条的待提炼数）。单条/少量提炼不必跑。
