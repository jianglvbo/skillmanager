...
- **Zero formula errors.** Never ship while `formula_refresh.py` reports `errors_found`. If you think an error predates you, prove it: load the *original* with `data_only=True` and look at that cell. An error you introduced looks exactly like one you inherited.
...
```bash
python scripts/formula_refresh.py output.xlsx [timeout_seconds]   # default 30
```
...
real, fails, writes `#NAME?`, and deletes every link. `formula_refresh.py` refuses to run in that state
...
- **`data_only=True` on a file openpyxl just wrote returns `None` everywhere** — run `formula_refresh.py` first. (A formula whose result is `""` also reads back as `None`.)
...
`openpyxl`, `pandas`, `markitdown` (pip, preinstalled — install only if an import fails or the command is missing) · LibreOffice (`soffice`, auto-configured for sandboxed environments via `scripts/ecmadoc/lo_runner.py`)
