...
def recalculate(filename, timeout=30, force=False):
    ...
    with tempfile.TemporaryDirectory(
        ...
        return _run_with_profile(filename, abs_path, timeout, Path(profile_dir))


def _run_with_profile(filename, abs_path, timeout, profile_dir: Path):
    started = time.monotonic()
    profile_url, err = _install_macro(profile_dir, timeout=timeout)
    if err:
        return {"error": err}

    timeout = max(5, int(timeout - (time.monotonic() - started)))

    before = _fingerprint(abs_path)
    ...
    elif platform.system() == "Darwin" and _have_gtimeout():
    ...
    try:
        result = subprocess.run(
            cmd, capture_output=True, text=True, env=build_office_env(), timeout=timeout + 15
        )
    except subprocess.TimeoutExpired:
        return {"error": timed_out}
    except FileNotFoundError:
        return {"error": NO_SOFFICE_MSG}
    ...
    if _fingerprint(abs_path) == before:
        return {
            "error": (
    ...
    try:
        ...
        for err_type, locations in error_details.items():
            if locations:
                entry = {"count": len(locations), "locations": locations[:MAX_REPORTED]}
                if len(locations) > MAX_REPORTED:
                    entry["locations_truncated"] = len(locations) - MAX_REPORTED
...
def main():
    ...
    if not args:
        print("Usage: python formula_refresh.py <excel_file> [timeout_seconds] [--force]")
