"""Command line tool that audits Office document XML against XSD schemas and
tracked-change integrity.

Usage:
    python doc_audit.py <path> [--original <original_file>] [--auto-repair] [--author NAME]
...
from partkit import OOXML_SUFFIX_FAMILY, extract_zip_safely, repack_zip
from auditors import DeckPackageAuditor, RevisionTracker, WordPackageAuditor
...
def main():
    ...
    if args.original:
        ...
        if original_file.suffix.lower() not in OOXML_SUFFIX_FAMILY:
            _fail(f"{original_file} must be one of: {', '.join(sorted(OOXML_SUFFIX_FAMILY))}")

    family = OOXML_SUFFIX_FAMILY.get((original_file or path).suffix.lower())
    if family is None:
        _fail(
            f"Cannot determine file type from {path}. Use --original or provide one of: {', '.join(sorted(OOXML_SUFFIX_FAMILY))}."
        )
    ...
    packed_file = None
    temp_dir_ctx = None
    if path.is_file() and path.suffix.lower() in OOXML_SUFFIX_FAMILY:
        packed_file = path
        temp_dir_ctx = tempfile.TemporaryDirectory()
        unpacked_dir = Path(temp_dir_ctx.name)
        try:
            with zipfile.ZipFile(path, "r") as zf:
                extract_zip_safely(zf, unpacked_dir)
    ...
    match family:
        case "docx":
            auditors = [
                WordPackageAuditor(unpacked_dir, original_file, verbose=args.verbose),
            ]
            if args.author is not None:
                auditors.append(
                    RevisionTracker(unpacked_dir, original_file, verbose=args.verbose)  
                )
        ...
        case "pptx":
            auditors = [
                DeckPackageAuditor(unpacked_dir, original_file, verbose=args.verbose),
            ]
        case "xlsx":
            exts = ", ".join(k for k, v in sorted(OOXML_SUFFIX_FAMILY.items()) if v == "xlsx")
            print(
                f"No XSD schema validation is performed for xlsx-family files ({exts}). "
                "For formula-error checking, use scripts/formula_refresh.py instead."
            )
            sys.exit(0)
        case _:
    ...
    if args.auto_repair:
        total_repairs = sum(v.apply_repairs() for v in auditors)
        if total_repairs:
            print(f"Auto-repaired {total_repairs} issue(s)")
            if packed_file is not None:
                repack_zip(unpacked_dir, packed_file)
                print(f"Wrote repaired file to {packed_file}")

    success = all([v.run_checks() for v in auditors])
