"""Format-specific checks for WordprocessingML packages."""
...
class WordPackageAuditor(PackageAuditor):
    ...
    def run_checks(self):
        ...
        if not self._check_whitespace_kept():
            all_valid = False
    ...
    def _check_whitespace_kept(self):
        errors = []
    ...
    def _count_paragraphs_new(self):
        count = 0
    ...
    def _count_paragraphs_orig(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                with zipfile.ZipFile(original, "r") as zip_ref:
                    extract_zip_safely(zip_ref, Path(temp_dir))
    ...
    def _report_paragraph_delta(self):
        new_count = self._count_paragraphs_new()
        if self.original_file is None:
            print(f"\nParagraphs: {new_count}")
            return

        original_count = self._count_paragraphs_orig()
        diff = new_count - original_count
        diff_str = f"+{diff}" if diff > 0 else str(diff)
        print(f"\nParagraphs: {original_count} → {new_count} ({diff_str})")

    def _as_int(self, val: str, base: int = 16) -> int:
        return int(val, base)

    def _check_id_limits(self):
        errors = []
            ...
            try:
                for elem in lxml.etree.parse(str(xml_file)).iter():
                    if val := elem.get(para_id_attr):
                        try:
                            if self._as_int(val, base=16) >= 0x80000000:
                                errors.append(
                    ...
                    if val := elem.get(durable_id_attr):
                        if xml_file.name == "numbering.xml":
                            try:
                                if self._as_int(val, base=10) >= 0x7FFFFFFF:
                                    errors.append(
                                        f"  {xml_file.name}:{elem.sourceline}: "
                            ...
                            try:
                                if self._as_int(val, base=16) >= 0x7FFFFFFF:
                                    errors.append(
                                        f"  {xml_file.name}:{elem.sourceline}: "
                                        f"durableId={val} >= 0x7FFFFFFF"
                                    )
    ...
    def apply_repairs(self) -> int:
        ...
        repairs += self._repair_durable_ids()
        return repairs

    def _repair_durable_ids(self) -> int:
            ...
            try:
                ...
                for elem in dom.getElementsByTagName("*"):
                    for attr_name in DURABLE_ID_ATTRS:
                        ...
                        try:
                            key = self._as_int(durable_id, base=base)
