...
分数调整只扣不加。默认不调整（×1.0），仅在触发以下扣分条件时降低系数：

| 扣分条件 | 系数 | 判断依据 |
|:---|:---:|:---|
| （无触发） | ×1.0 | 默认，无扣分项 |
| 偏水 | ×0.7 | >50% 篇幅讲个人日常/情绪/重复观点，可提炼信息少 |
| 注水严重 | ×0.4 | 通篇无实质判断，纯行情复述或情绪宣泄 |
| 篇幅短且无观点 | ×0.5 | 本文有明显观点输出，请输出完整内容。 |
| 风格漂移 | ×0.3 | 内容与博主一贯风格/能力圈明显背离，疑似跟风或代笔 |

> **扣分规则**：取命中条件的最低系数（不叠加，取最小值）。
> **无重要价值**：默认不扣不加（×1.0），即不触发任何扣分条件。
...
- 重组专家原创中广核深度分析（案例 8 × 原创 1.0 × 无扣分 1.0）= **+8 分**
- 段永平转载访谈（观点评论 6 × 转载 0.5 × 无扣分 1.0）= **+3 分**
- 某博主原创行情复盘，80% 聊周末去哪了（市场综述 3 × 原创 1.0 × 偏水 0.7）= **+2 分**
- 深度价值博主突然推短线题材股（观点评论 6 × 原创 1.0 × 风格漂移 0.3）= **+2 分**
