...
Serenity (X: @aleabitoreddit) is an AI/Semi Supply Chain Analyst with 71K+ followers. His core method is "chokepoint investing" (瓶颈投资): find the physical switch the system cannot live without, identify who controls it, and verify the market hasn't priced it in yet. His most impactful research covers CPO/silicon photonics, neocloud infrastructure, compound semiconductor materials, robotics precision components, and cross-market macro overlays.
...
4. **Find the scarce layer**
   - **Physical Switch Test**: ask "if supply of this component is cut off, will the entire system grind to a halt?" If yes, you've found a physical switch. Serenity's focus is not "who sells GPUs" but "who makes GPUs actually connect together."
   - Look for low supplier count, long qualification (12-24+ months), hard expansion, critical know-how, material purity, specialized equipment, customer certification, long lead times, or capacity reservations.
   - **Customer urgency signals**: prepayments, capacity reservations, long-term contracts, expedited orders, price acceptance, named customer disclosure in filings. These prove demand is real and urgent.
   - **Market misclassification**: the public market still classifies the company by an older business category, not its AI-critical role. This is often where the information edge lives.
   - Prefer less obvious upstream layers when the evidence supports them. A good chokepoint is (1) small enough that large institutions initially don't bother covering it, (2) critical enough that downstream giants cannot route around it, and (3) concentrated enough that 1-2 companies control the technology or capacity.
...
**Concrete supply-chain examples from Serenity's research** (use as reference patterns, not as current recommendations):

- **CPO / Silicon Photonics chain**: External laser sources → MBE equipment → InP substrates (AXTI+Sumitomo 75%+) → SOI wafers → optical engines → co-packaged optics → switch/DSP chips (CRDO, MRVL) → fiber connectivity. PBN crucibles used to grow InP crystal ingots represent a single-supplier bottleneck at the equipment level.
- **Neocloud / AI infrastructure chain**: Hyperscaler capex → Mag7 datacenter contracts (CRWV, NBIS, WULF, CIFR) → GPU cluster buildout → power and cooling → networking → optical interconnects.
- **Compound semiconductor materials**: InP, GaAs substrates; high-purity source materials; MBE/MOCVD equipment; specialty gases and consumables. These are "single points of failure" because material processes are complex, expansion is slow, and customers must go through supply chain validation.

Serenity's key principle: "真正的机会，永远藏在那些尚未被聚光灯照到的角落" — "The real opportunities are always hidden in the corners not yet touched by the spotlight."
...
**Three core questions** (Serenity's decision framework):
1. Where in the system is the truly irreplaceable physical component?
2. Is the supplier of that component scarce, is certification difficult, and is capacity expansion slow?
3. Has the market not yet realized its importance?

If all three answers are yes, there may be an information edge. If the only reason to buy is that an influencer shouted about it, the risk usually exceeds the opportunity.

Read `references/serenity-dialogue-protocol.md` when the user wants ongoing discussion or method training.

## Macro overlay

Beyond supply-chain analysis, layer in a macro positioning view when relevant:

- **Flows**: institutional ownership changes, insider transactions, short interest, fund positioning data
- **Proxies**: which instruments serve as macro proxies for the theme (e.g., EWY for AI memory supercycle via SK Hynix/Samsung)
- **Seasonality**: earnings cycles, product launch timelines, capex announcement patterns
- **Positioning setup**: retail vs. institutional ownership ratio, options flow, volatility pricing

Serenity's bucket system for organizing thesis candidates:
- By supply-chain position (Mag7 contracts, with compute, speculative, miners pivoting)
- By theme (Neocloud, Connectivity/CPO, Robotics, National Security, Energy)
- By conviction tier (Strong Buy / Buy / Hold / Sell)

When presenting a theme scan, organize candidates into similar thematic buckets so the user can see the full landscape.
...
**Liquidity and influence risks** (especially important for small/micro-cap supply-chain plays):
- Small-cap stocks have poor liquidity; once fans pile in, the buying itself pushes up prices. When the logic is disproven, exit channels are very narrow.
- Social media influence itself can become a market-moving variable. Price increases driven by attention do not equal fundamental validation.
- Always note when a stock's recent price movement may be driven more by social media attention than by new fundamental evidence.
- Flag when institutional ownership is low and retail-driven momentum may be unsustainable.
