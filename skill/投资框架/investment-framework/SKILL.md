...
description: >
  ...
  定义流水线（粗制品→粗加工→原始资源→提炼方案→用户确认→提炼执行→审查）、模板表、路径表、全局规则、审查机制。
...
**第一步：提炼分析**——读取工作区/原始资源/ 下的文件全文，分析内容后生成提炼方案报告。报告用「内容卡」结构（📥提炼什么／📤提炼成什么／✅质量自评／💡我的建议／⚠️待你定 + 顶部总览表），详见 investment-refine SKILL。单篇默认对话直出（不写文件）；批量「提炼所有」默认写入 `{PROPOSAL_DIR}/YYYY-MM-DD-提炼方案.md`（含顶部「⚙️决策区」供勾选）。
...
**第四步**：输出审查报告，标记问题项
**第五步（待回收处置 · 默认执行）**：审查自动读取 `RECYCLE_CONSOLE`（`工作区/待回收控制台.md`），按全局规则 #26 处置超期条目（真删 + 双向清理）并出「待回收处置报告」给出理由；未到期条目填 `状态` 为 `待回收（剩 N 天）`
...
| PROPOSAL_DIR | {VAULT_ROOT}/工作区/提炼方案 | 提炼方案文件（批量「提炼所有」时写入，含顶部决策区） |
...
| 提炼分析 | 提炼方案报告（内容卡结构） | 对话直出 / 写 `{PROPOSAL_DIR}`（批量） |
