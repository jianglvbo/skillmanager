...
description: >
  ...
  定义流水线（粗制品→粗加工→原始资源→提炼）、提炼预览表、模板表、路径表、全局规则、审查机制。
...
- **三层归属**：我的（用户自管）/ 博主（已登记）/ 其他（未登记投资人），宏观不区分归属层。
- **提炼预览表驱动**：粗加工生成预览表，提炼按预览表执行。一篇帖子可拆为多条框架条目。
...
| 仅提炼 | 提炼 | investment-refine（需已有原始资源） |
| 审查 | 审查、review、健康度 | investment-review |
| 查看全貌 | 投资框架、框架全貌、pipeline | 输出框架说明 |

### 粗加工 → investment-coarse-processor

**第一步**：读取粗制品文件（工作区/粗制品/ 下的原始帖子内容）
**第二步**：整理格式、去广告、补全 metadata（title/source/author/date/type/status）
**第三步**：在文件底部生成「提炼预览」表格（见 references/preview-table-rules.md）
**第四步**：将文件从 工作区/粗制品/ 移动到 工作区/原始资源/
**第五步**：自检——预览表是否覆盖帖子中所有有价值内容？

### 提炼 → investment-refine

**第一步**：读取工作区/原始资源/ 下的文件 + 底部提炼预览表
**第二步**：按预览表逐行处理——一篇帖子可拆为多条框架条目
**第三步**：对每行确定归属层（我的/博主/其他）和分类文件夹，从模板表选择对应模板
**第四步**：创建文件，填写 frontmatter + 正文内容
**第五步**：如涉及已登记博主，同时更新 `博主/{博主名}/{博主名}.md` 的信息汇总
**第六步**：如涉及宏观事件，创建/更新宏观文件并关联行业/个股
**第七步**：自检——所有预览表行是否都已处理？
...
| MACRO_DIR | {VAULT_ROOT}/宏观 | 宏观事件（不区分归属） |
...
| 步骤 | 输出 | 位置 |
|:---|:---|:---|
| 粗加工 | 整理后的原始帖子 + 提炼预览表 | {RAW_DIR}/{标题}.md |
| 提炼 | 框架条目（≥1个文件） | 归属层/分类/{文件名}.md |
| 审查 | 审查报告 | 输出到对话中 |
