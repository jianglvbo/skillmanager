...
26. 待回收控制台与 7 天冷静期回收：框架条目若你觉得质量不佳需回收，记入 `工作区/待回收控制台.md`（路径见 investment-framework 路径表 `RECYCLE_CONSOLE`）。
   - **你只填三栏**：`条目`（wikilink）、`添加日期`（yyyy-MM-dd）、`理由`；`状态` 栏留空，由下一次审查计算填入。
   - **撤销 = 删行**：添加日期起 7 天内，你随时可直接删掉该行撤销回收，无任何副作用，条目保持原样。
   - **超 7 天自动回收**：审查时计算 `today - 添加日期 > 7天` 的条目，执行**真删**（文件移出 vault 至系统废纸篓）+ **双向清理**（反查并清除 vault 内所有指向该条目的 wikilink 与关联脚注，含博主档案、其他条目、`source` 字段），保持无悬空链接、不制造孤岛文件。
   - **状态栏由审查填入**：`待回收（剩 N 天）`（未到期）/ `已回收`（已执行，该行作为历史记录保留）。
   - **审查报告须给理由**：每条回收在「待回收处置报告」中写明「超 7 天冷静期 + 用户原始理由 + 清理引用数」，供你复核。
   - **Agent 严格自律**：绝不替用户在控制台预填条目或填 `状态` 栏；绝不缩短 7 天冷静期；超期回收必出理由报告。回收动作严格由「用户填三栏 + 审查超期执行」驱动，Agent 不得单方面发起回收。
