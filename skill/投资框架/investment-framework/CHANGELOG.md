...
## 2026-08-01
- `investment-framework/SKILL.md`（v2.4.5）：
  - 「按需创建」核心原则由「仅博主文件夹」扩充为全局规则：任何层级目录都不预建空文件夹，有内容写入时才创建；发现空文件夹应清理（「我的」层由用户自管，不主动删）
  - 「禁止行为」同步扩充：绝不预建或保留任何层级的空文件夹
  - 根因：vault 积累多处空文件夹（博主层投资心得、工作区/提炼方案、附件/图片 子目录等），已清理；规则前置约束避免再积累
