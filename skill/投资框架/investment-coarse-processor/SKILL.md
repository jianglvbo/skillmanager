...
- **绝不输出模板指引文本/占位符**（footnote-taxonomy 禁止行为 #5 · 2026-08-08 用户纠正）：模板中的花括号写作指引（`{...}`）与 `> 指引 blockquote`（如"原文链接必填真实雪球帖子 URL"、"示例：淘宝闪购单量追上美团"）仅为提示，**产出文件中不得包含任何花括号指引文本或模板指引 blockquote**——轻创建画像时只保留 section 标题 + 空表格骨架，指引/占位符全部剔除
