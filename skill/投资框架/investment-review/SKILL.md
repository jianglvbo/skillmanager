...
- **待回收处置例外**：全局规则 #26 的回收执行（真删 + 双向清理）属于用户经 7 天冷静期已授权的既定动作，审查直接执行并出「待回收处置报告」给出理由；其余审查发现（关联备注等）仍仅报告不修改
...
**R5**：输出「待回收处置报告」，**必须给出每条回收的理由**（超 7 天冷静期 + 用户原始理由 + 清理引用数）；未到期者提示剩余天数
...
- [ ] 待回收处置 R1-R5 是否执行？（读取控制台、按 7 天冷静期判定、超期者真删 + 双向清理、状态栏已填入、报告给出理由）？
