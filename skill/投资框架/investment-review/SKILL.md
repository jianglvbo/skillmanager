...
**C5**：我的 vs 博主冲突检查——"我的"层的方法论 vs "博主"层的方法论是否有冲突
...
**R1**：解析控制台表格，逐行提取 `条目`(wikilink) / `添加日期` / `理由` / `状态`(可能为空)
**R2**：对每行计算 `冷静天数 = today - 添加日期`
**R3**：分支判定
- 若 `状态` 已为 `已回收` → 跳过（历史记录，不重复处理）
- 若 `冷静天数 > 30` → 标记「本次将回收」，执行 R4
...
**R5**：输出「待回收处置报告」，**必须给出每条回收的理由**（超 30 天冷静期 + 用户原始理由 + 清理引用数）；未到期者提示剩余天数
...
- [ ] 内容审查 C1-C9 是否全部执行？（读取参数、扫描、一致性、知行合一、我的vs博主、经验验证、关联备注、关系依据复核、输出报告）
...
- [ ] 待回收处置 R1-R5 是否执行？（读取控制台、按 30 天冷静期判定、超期者真删 + 双向清理、状态栏已填入、报告给出理由）？
