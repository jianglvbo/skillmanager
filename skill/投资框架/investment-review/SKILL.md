...
**控制台**：{recycle_console_path}
**本次回收**：{k} 条 ｜ **冷却中**：{m} 条 ｜ **历史已回收**：{h} 条

| 条目 | 添加日期 | 冷静天数 | 处置 | 理由（审查给出） |
|:--|:--|:--|:--|:--|
| [[条目A]] | 2026-06-01 | 39 | ✅ 已回收 | 超 30 天冷静期；用户原理由：「逻辑跳跃、无数据支撑」；双向清理 3 处 inbound 引用（博主档案 1 + 其他条目 2），无悬空链接 |
| [[条目B]] | 2026-07-05 | 5 | ⏳ 待回收（剩 25 天） | 冷静期内，保留；用户原理由：「暂不确定是否有用」 |
...
| 段落布局预扫 | investment-framework/references/verify-format.py | 段落布局/脚注内联/模板废话扫描（可 --fix 自动修复） |
