...
- **待回收处置例外**：全局规则 #26 的回收执行（真删 + 双向清理）属于用户经 30 天冷静期已授权的既定动作，审查直接执行并出「待回收处置报告」给出理由；其余审查发现（关联备注等）仍仅报告不修改
...
**第七步**：关联备注——为缺少跨条目关联的条目补充脚注（脚注类型和格式见 `investment-framework/references/footnote-taxonomy.md`），在正文相关论述处嵌入标记，文末脚注定义写 wikilink + 关系类型 + 一句话说明
...
**第一步**：读取参数 `{ scope_dirs }`
**第二步**：归类正确性——含博主层条目其作者是否均在「博主控制台」登记，未登记者误挂博主层须标记迁移至其他层（见 framework-rules #12）
...
读取 `工作区/待回收控制台.md`（路径见 investment-framework 路径表 `RECYCLE_CONSOLE`），按全局规则 #26 处置待回收条目。

**第一步**：解析控制台表格，逐行提取 `条目`(wikilink) / `添加日期` / `理由` / `状态`(可能为空)
**第二步**：对每行计算 `冷静天数 = today - 添加日期`
**第三步**：分支判定
- 若 `状态` 已为 `已回收` → 跳过（历史记录，不重复处理）
- 若 `冷静天数 > 30` → 标记「本次将回收」，执行第四步
- 若 `冷静天数 ≤ 30` → 将 `状态` 填为 `待回收（剩 N 天）`，跳过
**第四步（回收执行 · 真删 + 双向清理）**：对超期条目
...
- 将条目文件移出 vault（移至系统废纸篓，完成真删）
- 将该行 `状态` 填为 `已回收`，记录清理了 M 处引用
**第五步**：输出「待回收处置报告」，**必须给出每条回收的理由**（超 30 天冷静期 + 用户原始理由 + 清理引用数）；未到期者提示剩余天数
...
- [ ] 待回收处置：是否读取控制台、按 30 天冷静期判定、超期者真删 + 双向清理、状态栏已填入、报告给出理由？
