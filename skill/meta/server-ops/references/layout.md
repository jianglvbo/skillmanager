...
- **业务库 investment_kb**：**16 表**（单 dict 表存全部码值 + 业务表，含预测域 4 表；2026-08-31 dict 14→1 整合），DDL 权威在 `~/Ai/tools/investment-kb/investment_kb.sql`（~/Project 副本可能滞后）
...
- 注意：investment-console **重度使用 MySQL**（vault 派生表 files/tags/bloggers + 运营表 refine/review/coarse/prediction 等）；fitness-console 亦用 MySQL
