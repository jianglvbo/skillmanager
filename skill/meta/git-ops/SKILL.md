...
- **★ 改完就提交，不必等指令（2026-09-15 用户约定）**：agent 自己动过的文件，改完**随手提交**，不要等用户说「提交」——用户原话「你有改动，git 自己记得提交，push 按照 git 规则来，满 5 个 push 一次」；推送阈值见第六步（未推送 commit ≥5 → 推一次），本 skill 是该阈值的唯一真相源，其它 skill/README 不重复定义。
