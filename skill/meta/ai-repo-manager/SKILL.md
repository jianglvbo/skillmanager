...
- **★ 仓库冻结（2026-09-15 用户硬约束）**：仓库**只维护现有的这批 skill**——除非用户明确要求，**不得新增任何 skill 进仓库**。用户原话：「仓库就只有管理仓库内的现在这些 skill，除非我要求，不然不要新增 skill 进去」。采集/整理/同步过程中发现的本地 skill、商店下载的 skill，一律不要顺手收进仓库；要加先问一句。
...
- **绝不自行新增 skill 进仓库**（2026-09-15 用户硬约束）——只维护现有清单，要加必须用户明确要求
