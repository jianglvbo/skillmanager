  ...
  "targets": [
    {
      ...
      "thinking": [                            // 思考链：标准 4 步（识别/判断/归类/生成）
        "识别：<从原文哪部分发现该内容点>",
        "判断：<该内容点的价值/可复用性>",
        "归类：<为什么归此分类/归属层>",
        "生成：<新建/追加的产物路径>"
...
- **思考链（thinking）规范**：每条产物必须生成标准 4 步思考链（2026-08-17 用户确认）——① 识别（从原文哪部分发现该内容点）② 判断（该内容点的价值/可复用性）③ 归类（为什么归此分类/归属层）④ 生成（新建/追加的产物路径）。**每一步来自第一步分析已产出的真实判断**（归属层铁律、标签体系、同作者预检），禁止事后编撰；看板决策链路图按此渲染 agent 思考轨迹
