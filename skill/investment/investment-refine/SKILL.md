...
> **写入前必过**（不要等到自检环节）：每条言论在调用 `blogger_statement` 前，先答完八问。答错会被**服务端拒写**（`_validateSignal` / `form` / 空壳 / sourceUrl 硬门禁），返工成本高。
...
| **⑧** | **八项必有字段齐了吗？**（2026-09-11 用户确认） | 内容时间 `viewDate`（判断成立时点，缺省=帖子时间）· 帖子时间 `postDate` · 内容类型 `contentType` · 帖子类型 `form` · 回复 `replyTo`（`form=回复` 必带）· 信号 `stance`+`signal`（预测必带方向）· 原文链接 `sourceUrl` · 采集时间 `fetchedAt`（抓取日期，缺省今日；**提炼历史批次要传该批次采集日**）。三个时间别混：帖子时间＝发帖、内容时间＝判断成立、采集时间＝我们抓的日期 |
