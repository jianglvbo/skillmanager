...
### 第 1.5 步：博主言论分流决策矩阵（2026-09-03《博主言论设计》+ 方案 v2 §4）
...
**`predict` 判定三要素（2026-09-04 用户定义）**：① 明确方向（看多/看空/中性）② 未来指向（时间窗："几年后/几个月后/年内"，或事件条件："美国加息/降息""地缘冲突缓和"等）③ 可判对错（含目标位/幅度/点位最佳）。缺任一要素即回落 `view`（当下判断）或 `insight`（复盘心得）；历史复盘叙述**不得**算预测。
