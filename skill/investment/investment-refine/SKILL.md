...
> **写入前必过**（不要等到自检环节）：每条言论在调用 `blogger_statement` 前，先答完六问。答错会被**服务端拒写**（`_validateSignal` / `form` / 空壳 / sourceUrl 硬门禁），返工成本高。
...
> **批量外部判断的格式风险（2026-09-11 教训）**：把提炼外包给子任务时，务必在任务书里写明「signal 不带方向词前缀」「form 必填」「正文不写 wikilink」——否则会成批产出「信号：看多 · 看多…」这类需要事后返工的数据。
