...
> **写入前必过**（不要等到自检环节）：每条言论在调用 `blogger_statement` 前，先答完七问。答错会被**服务端拒写**（`_validateSignal` / `form` / 空壳 / sourceUrl 硬门禁），返工成本高。
...
**服务端门禁**（写不进去的 6 类）：无方向却有 signal / signal 自带方向词前缀 / 占位符 `—` / 元描述 / 话题标签 / 超 25 字；另加 `form` 必填、正文与信号不能同时为空、`sourceUrl` 必填。
