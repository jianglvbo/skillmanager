...
> **写入前必过**（不要等到自检环节）：每条言论在调用 `blogger_statement` 前，先答完三问。答错会被**服务端拒写**（`_validateSignal` 硬门禁），返工成本高。
...
| **③** | **信号够短吗？** | 超 25 字 → 精简为 `方向 · 具体结论`，用 `/` 分隔并列要素 |
...
**服务端门禁**（写不进去的 5 类）：无方向却有 signal / 占位符 `—` / 元描述 / 话题标签 / 超 25 字。
