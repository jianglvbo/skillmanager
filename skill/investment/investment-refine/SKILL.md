...
用户原话：「显示你无法处理的需要我复核的帖子，这种帖子在下次审查的时候可以处理，并且内化规则，**让我以后可以不用再审核类似的帖子**」。提炼时遇到**规则没覆盖 / 两种归类都说得通 / 标的名认不出**的情况，**不许瞎猜、不许留空糊过去**，调 `MCP pending_review action=add` 上报（看板「待复核」页在「审查」左边）：
