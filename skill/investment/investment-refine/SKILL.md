...
5. 如涉及已登记博主，**言论落位双落地（2026-09-06 用户拍板）**：读 `博主/{博主名}/{博主名}.md` 后**不手改**画像言论表，改为调用 MCP `blogger_statement`（action=add，contentType 按六分法，view 内附具象化指针 `见 [[博主/{名}/{分类}/{文件名}]]` 完整路径 wikilink，viewDate=言论日期，涉个股/行业/市场必传 subjectId，subjectId 缺失先调 `console_ensure_subject`）——MySQL 权威 + 服务端自动镜像画像 md，两边同时落地；手改画像表格只在 MySQL 不可用时兜底并汇报。**具象化指针必须用完整路径**：`见 [[博主/{名}/{分类}/{文件名}]]`，禁止 `见 [[文件名]]`
