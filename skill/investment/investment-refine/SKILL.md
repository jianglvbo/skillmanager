...
> **滑块处置（2026-09-21 用户拍板）**：提炼中经 ego-browser 打开的页面（③ 回采、查证、核验原文）命中滑块/安全验证时**不许后台静默重试**——立即把该标签页置前交用户：对命中页做一次页面操作使其切前台（ego 页面操作默认激活该页）→ `open -a com.citrolabs.ego.lite` 拉起应用 → `task.handOff()`；用户过完验证后接管重试本页。任务收尾仍按 ego-browser skill `finish({keep:[]})` 回收空间。
>
...
- **博主言论落库**：写入前必过**八问**（有方向吗 / 信号具体吗 / 够短吗 / 正文干净吗 / `form` 填了吗 / 篇幅砍过头吗 / 新称呼落库吗 / 九项字段齐吗）与三份合法信号范例、服务端门禁清单——**完整表格见 `references/refine-schema.md` §七**。答错会被服务端拒写（`_validateSignal`/`form`/空壳/`sourceUrl` 硬门禁），返工成本高。
...
向用户报告：创建了哪些条目（标题 + 分类 + 归属层）、是否更新博主档案、条目间关联方向、可验证判断提示（第二步第 6 条）、产出校验结果、待决策队列处理结果（第一步 1.1 第 5 条）。**批量提炼（帖子积压很多、分多轮推）时每轮汇报末尾跑一次 `node scripts/refine-progress.js`**（读 `config.json` 的 mysql，只读统计 `post_history`）贴出 ASCII 进度：总进度条 =「已处理/总量」，另附**本轮质量条** = 自上次运行增量里「已落言论 vs 判无价值」占比（基线存 `scripts/.refine-progress.state.json`，已 gitignore），用来看这一批提炼得怎么样。单条/少量提炼不必跑。
