...
| **⑧-2** | **新称呼落库了吗？**（2026-09-12 用户要求） | 本批若出现**未登记的个股称呼**（寒王/讯狗/小米 之类）→ 调 `stock_alias(action=add)` 写入 `stocks.aliases`；若该称呼**本身是常用词**（美的/小米/平安）→ 再调 `mark-ambiguous` 落 `dict(type=ambiguous_word)`；顺手 `set-keywords` 补主营词。判定规则见 `references/stock-mention-rules.md` |
