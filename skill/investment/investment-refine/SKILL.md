...
| **⑧** | **九项必有字段齐了吗？**（2026-09-11 用户确认；**「必有」＝字段存在，不是必须填**） | 内容时间 `viewDate`（判断成立时点，缺省=帖子时间）· 帖子时间 `postDate` · 内容类型 `contentType` · 帖子类型 `form` · 回复 `replyTo`（可空）· 信号 `stance`+`signal`（可空）· 原文链接 `sourceUrl` · 采集时间 `fetchedAt`（抓取日期，缺省今日；**提炼历史批次要传该批次采集日**）· 具象化 `wikiRef`（**填 vault 相对路径**（含 `.md`），如 `博主/X/条目名.md`——卡片点「已具象化」即本地打开；**不要填条目名**；**可空**，已具象化时服务端按 vault 笔记的 source 链接自动反查回填）。**其中 回复 / 信号 / 具象化 三项允许为空**（无回应对象、无方向、未沉淀条目），只有 帖子时间 / 内容类型 / 帖子类型 / 原文链接 必须非空。三个时间别混：帖子时间＝发帖、内容时间＝判断成立、采集时间＝我们抓的日期 |
...
5. **不写任何博主画像 md**（2026-09-12 用户决定：画像 md 废弃、内容只在看板）——涉及已登记博主时，言论按上文落库到 `posts` 六表即可，看板自动展示；**具象化指针（`wikiRef`）用完整路径**：`见 [[博主/{名}/{分类}/{文件名}]]`，禁止 `见 [[文件名]]`
