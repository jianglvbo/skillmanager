...
> **`//@` 切分（2026-09-09 固化；2026-09-11/12 两次修正）**：回复类帖子按 `//@` 切分——**第一个 `//@` 之前是博主原创（提炼对象）→ `view`**；`//@` 之后是被回应者的话 → **`replyTo` 字段**（**只存剥掉 `//@昵称：` 与嵌套 `回复@昵称:` 包装之后的原话本身**，说话人昵称不进字段；**不再写成正文里的 `（回应"…"）` 括号摘要——该写法 2026-09-11 起废弃**）；`//@` 后署名 = 当前博主时属博主旧话，可提炼（注意去重）。完整规则见 post-fetch `references/refine-checklist.md` §2 与 framework-rules #46（提炼前必加载）。
...
> `post_history` 的用途有二（2026-09-12 用户拍板）：① **提炼的原文来源**（帖子集路径直接从这里读，不再落 vault 文件）；② **避免重采**。它不参与归类判定、也不存提炼产物，提炼产物仍落 `statements`（六表 + 视图；言论行带 `post_history_id` 指回留档，方向是**言论表 → post_history**）。
>
...
5. **不写任何博主画像 md**（2026-09-12 用户决定：画像 md 废弃、内容只在看板）——涉及已登记博主时，言论按上文落库到 `statements` 六表即可，看板自动展示；**具象化指针（`wikiRef`）用完整路径**：`见 [[博主/{名}/{分类}/{文件名}]]`，禁止 `见 [[文件名]]`
