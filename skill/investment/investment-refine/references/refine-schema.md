...
| ⑤ | `replyTo` 回复 | 被回应的对方**原话** | **可空**（无回应对象/采集原文无 `//@` 段则留空）；有则**原样照抄** `//@某人：` 之后的全部内容（2026-09-12）。**形态先看留档**：`form` 必须以 `post_history.form` 为准，标错成「回复」会让卡片去找不存在的被回应者原话（2026-09-12 修了 43 条这类误标） |
...
| **回应** | **`replyTo`** | text（**不限长**，2026-09-12 由 varchar(500) 改 TEXT） | **被回应者的原话**——`//@某人：` 之后的全部内容**原样照抄**；**不得再内嵌进 `view`** |
| 信号 | `stance` + `signal` | enum + text | 方向 + 具体结论 |
| 原文 | `sourceUrl` | url | — |
| 已具象化 | `wikiRef` | text | 框架条目名 |

> **`//@` 切分落位（配套规则 v2，2026-09-12 用户拍板「回复的内容不要提炼，直接原文塞到回复里面」）**：第一个 `//@` **之前** = 博主本人原话 → `view`；**之后** = 被回应者的原话 → `replyTo`，**一律原样照抄全部内容**：
> 1. **禁止提炼/概括/改写**——不得把「财主，当下的腾讯，您怎么看」写成「「当下的腾讯怎么看」」，不得把一段质疑写成「对其看好泰它西普的质疑」；
> 2. **禁止截断、禁止省略号**——旧写法末尾加 `…` 截到 50/500 字属违规（列已改 TEXT，实测最长 1647 字）；
> 3. **保留说话人**：写成 `//@昵称：原话`（昵称保留，它说明「回的是谁」；`回复@昵称:` 这类嵌套前缀属原话的一部分，一并保留）；
> 4. **没有就留空**：采集原文里没有 `//@` 段时留空（**不许用概括顶上**）——看板会显示「被回应者内容未采集（点编辑可补录）」，这才是正确信号；
> 5. **自回复也算**：`//@` 引的是博主本人（uid 相同）时为自回复链，照抄他本人旧话即可。
> **批量回填工具**：`investment-console/scripts/backfill-reply-to.js`（从 `post_history.raw_text` 抽 `//@` 段，空值写入 / 截断升级 / 概括改写，改写明细存 `backups/reply_to_verbatim_audit.tsv`）。
