...
| **示例（用户认可的写法）** | `信号：看多 · 白酒跑赢科技 / 未来半年到一年`<br>`信号：看多 · 看多铀`<br>`信号：短期看空 · 创业板破位后反弹结束` |
...
| **⑦ 无信号不填** | 信号是**可选**的：原文无方向立场 → **留空**（页面不显示信号行），**不得强行编造方向**。<br>`trade`/`predict` 通常含方向（买卖/预测天然有立场），但**若原文确实中性/无方向也可留空** | `neutral` 与「留空」不同：前者是「明确不偏多空」，后者是「未表达方向」 |
