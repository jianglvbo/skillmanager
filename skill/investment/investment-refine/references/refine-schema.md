...
|:---:|:---|:---|
| **1** | `trade` 买卖记录 | ① 含买卖动作（买入/卖出/加仓/减仓/清仓/换仓），或 ② 含**仓位/持仓/杠杆状态**（"无融资仓位""满仓""已清空""仓位降到X成"），或 ③ 含明确价格/市值等交易要素 |
| **2** | `predict` 预测记录 | 可验证的**未来判断**：方向 + 未来指向（时间窗或事件条件）+ 可判对错，三要素齐备 |
| **3** | `research` 研究 | 含**分析过程或数据支撑**（财报/公告/产业链/估值测算/调研/对比），非纯结论 |
| **4** | `insight` 心得总结 | **方法论/纪律/原则/复盘**——可复用经验 |
...
**信号（`stance`）判据（2026-09-11 固化）**：`stance` **仅在 `trade`（买卖方向）与 `predict`（预测方向）必填**；`view`/`insight`/`chat` 不填（看板亦只在买卖/预测卡片显示方向）。判据如下：
...
| **⑦ 禁止空值** | `trade`/`predict` 的 stance **不得为空**；历史 `null` 属违规，须回填 | `trade + stance=null` = 违规数据 |
