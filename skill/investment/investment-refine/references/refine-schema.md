...
**九项必有字段（2026-09-11 用户确认，服务端已硬门禁 + 自动反查）**：每条言论都必须能回答「什么时候的判断 / 什么类型的内容 / 什么形态的帖子 / 在回应谁 / 什么信号 / 原文在哪 / 什么时候采的 / **具象化到了哪个文件**」：
...
| ⑤ | `replyTo` 回复 | 被回应的对方原话/话题 | **`form=回复` 时必填**（服务端拒写）；取 `//@` 之后的内容或被回应者核心观点 |
| ⑥ | `stance`+`signal` 信号 | 方向 + 具体结论 | 有方向才填；**`contentType=predict` 必带方向**（预测三要素之一） |
| ⑦ | `sourceUrl` 原文链接 | 原帖 URL | **必填**（规则 #35） |
| ⑨ | `wikiRef` 具象化 | **本条被提炼进了哪个框架条目文件**（笔记名） | **已具象化则必填**；未具象化留空。写库时服务端会自动反查 vault（笔记 frontmatter `source` 里的原帖 URL）并回填，人工漏填也不会丢——但提炼时若刚创建/追加了条目，应主动填上；对应条目已删除时要清空 |
