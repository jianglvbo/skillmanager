...
51. 言论「待读/已读」是**用户的阅读状态**，agent 不碰（2026-09-12 用户要求）：
    ...
    - **菜单一律图标化（2026-09-15 用户要求）**：「修改所有的菜单，改成用 svg 的图标样式，中文只有当鼠标悬浮才展示」——左栏主导航、待决策三态 tab、言论追踪五维度 tab、言论分类 tab（全部/买卖记录/研究/预测/…）全部只渲染 SVG 图标，中文只存在 `data-label` 里，靠 CSS `::after` 悬浮弹出（**不要再往按钮里写字，也别用原生 `title`**，否则会双提示）。两种角标语言：右上红胶囊＝待读数（`.nav-badge`）、右下灰圆片＝条数（`.ico-num`）或双模 tab 的筛选状态（`.seg-chip`：个股市场 `.mkt-chip` 全/A/港/美、博主关注 `.bgf-chip` 全/★/☆）。分类 tab 条数为 0 时图标压暗（`.seg-empty`）。改动入口：`web/index.html`（静态 tab）+ `web/app.js` 的 `MENU_ICON`/`catTab()`/`setStockTabLabel()`。
