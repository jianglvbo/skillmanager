...
30. 博主档案「言论追踪」分类规范（2026-09-03 由四类 `kind` 改为内容五分，2026-09-04 增「预测记录」成六分）：按 `content_type` 分 **5 个 `###` 子表**（时间倒序，最近在上）——**研究 / 预测记录 / 观点 / 心得总结 / 闲聊**；买卖记录走 #31，不在此内。各子表统一列：观点时间 | 发帖时间 | 内容 | 标的/行业 | 方向 | 观点（博主判断） | 信号 | 来源 | 原文链接。
    - **研究**：含数据/估值/行业结构的可复用分析。若已沉淀框架文件，观点列写 `见 [[分类/文件名]]`（画像记事实、wiki 记方法，不重复）。
    ...
    - **双时间**：`观点时间` 默认等于 `发帖时间`（此时发帖时间列显示 `—`）；原文含"三年前/今年五月我的预判"等相对表述时按发帖时间折算，并把原句抄进「观点时间依据」备查。**硬约束 观点时间 ≤ 发帖时间**；推算跨度 ≥2 年或表述模糊 → 标（待复核）。粒度按表述给，不得把"三年前"写成精确到日。**「观点时间依据」只随 `derived`/`explicit` 存在**，`as_posted` 一律留空（服务端强制清空）：它记的是博主原话里的时间表述，不是数据出处——迁移/采集溯源走 `prediction_tracks.migrated_to_statement_id` 等映射字段，禁止塞进这里（2026-09-04 清掉 215 条被误写行的卡片噪声）。
...
35. 博主档案三表「原文链接」统一规范（治本防悬空/丢失）：博主画像的**三张追踪表**——言论追踪 5 子表、个股买卖记录、预测记录（准确率追踪）——**一律含「原文链接」列**（模板见 `assets/博主.md`），且**任意行原文链接禁止为空 `-`**。
    - **粗制品三种来源都必须有原文链接**（2026-09-03 用户明确）：① 微信截图进件；② `xq-post-fetch` 雪球博主言论采集；③ 其他正常粗制品。截图类须在录入前补到该言论的原始帖子链接，链接缺失即来源不可回溯。
...
38. vault 链接渲染与主题归类规范（2026-09-03 用户提出）：
    ...
    - **行业主题全量预建（2026-09-06）**：行业维度已按申万2021版**全量预建 35 个一级（31 申万，含综合）+ 4 自定义，及各一级下 125 个二级**（二级主题命名 `一级/二级`，如 `食品饮料/白酒`；来源 `init_sw_industries.js`）。**涉行业的言论/预测，选最精确的层级挂靠（二级优先，无精确二级才落一级），禁止自创行业主题名**——预建后行业维度再新建即属违规；存量非申万主题已于 2026-09-06 全部并转停用（快照 `outputs/industry_fix_snapshot.json`）。
    - **旧主题维（`prediction_tracks`）过渡语义**：该表自 2026-09-03 起为**迁移中的遗留表**，言论权威已归 `blogger_statements` 单轨（挂 `subject_id`）。B3 按"只标记不删行"迁回可归因行（`migrated_to_statement_id` 非空即不再在控制台展示，可回退）；未能归因（content 无 `[[博主/…]]` wikilink）的行**保留在旧表并标待复核**，控制台以「待清理 N」角标显示，归零后该主题自动下架（`enabled=0`）。**禁止为图干净而删除或隐藏仍承载待复核数据的主题**——那会让数据在 UI 上不可达。新增言论一律走 `blogger_statement`，不得再写 `prediction_tracks`（该表仅保留"对某条预测的后续增强/反驳"本职用途）。
    - **legacy 归因的两条证据（2026-09-03 用户纠正，路径 B 是原本就该用的强证据）**：① content 内的 `[[博主/{X}/…]]` wikilink 且 X 已建档；② **同一段文字已存在于某博主画像的言论表**（即 `blogger_statements` 里有同文行）——画像/DB 已在谁名下，该段就是谁的言论，归属者即该行博主。路径 ② 的处置是**合并指向既有行**（`migrated_to_statement_id` 指向该 statement 并补其 `subject_id`），**绝不新建一份**：同一句话在言论与旧主题维各存一份即为违规。两条路径都要求归一化全文相等（或长文本前缀包含）且**博主唯一**；多博主同文（转发/引用）一律不猜、留待人工。文本比较须去空白与中英标点后再比。③ **第三条确定证据**：采集批次名常为 `雪球采集-{博主昵称}-{日期}`，其中若**只命中一位**已建档博主即为确定归属（不算猜测）；命中多位或不命中仍不猜。按上述三条证据链，B3 共归位 720/843（85%），残余 123 条留旧表待人工复核。
