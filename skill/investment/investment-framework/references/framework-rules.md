...
40. 看板卡片展示一致性（2026-09-11 用户发现「铝」案例后确认）：**同一条言论在任何入口——言论追踪 / 博主详情 / 预测控制台时间线 / 买卖记录卡——必须展示同样的要素**：博主正文、**回应**（`reply_to` 字段）、信号行（`方向 · 内容`）、形态标记（`form`）、原文链接；**不得因所属类型不同而丢掉其中任何一项**。
    ...
    - **时间也是必备要素（2026-09-13 用户反馈「这些言论为什么没有时间」后补充）**：卡片顶行必须展示**内容时间 `view_date`（缺省＝帖子时间）**；两者不等时并排显示「内容 X · 言论 Y」，相等时只显示一个（同 #30 画像表「观点时间默认等于发帖时间」的口径）。**任何入口、任何卡片类型都不得出现没有时间的卡片**。根因案例：买卖记录卡只渲染 `statement_trade.trade_date`，而**该列允许为空**（实测 130 条里 86 条为空）→ 这些卡片顶行整格空白，看起来就是「没有时间」；`TRADE_SEL` 连 `view_date`/`statement_date` 都没查，卡片根本拿不到双时间。修复契约：`tradeCard` 复用与 `stmtCard` 同一个 `dualTime()`；买卖卡的**内容时间取买卖日期 → `view_date`**（#51 的时间回退链明确含「买卖时间」），**帖子时间取 `statement_date` 兜底**，两者都缺也不会空白；`TRADE_SEL` 必须带 `view_date`/`view_date_source`/`statement_date`。验收：渲染后逐卡断言顶行 `.cs-date` 非空（每个分类 tab 都要过一遍）。
