...
38. vault 链接渲染与主题归类规范（2026-09-03 用户提出）：
    ...
    - **言论单轨（2026-09-07 起；2026-09-08 分表，见 #39）**：博主言论唯一权威存储 = 按类型分表的 `post_*` 六表（`statements` 为只读 UNION 视图，挂 `subject_id`）；`prediction_tracks` 不再承载言论，仅保留"对某条预测的后续增强/反驳"本职用途（`console_add_track`），此类行在看板「预测跟踪」分组展示。**新增言论禁止写 `prediction_tracks`**。
...
44. 数据库结构与命名规则（2026-09-11 拍板，2026-09-12 终版：术语＝言论、实体三表、关联六表）：
    ...
    - **读取契约**：言论经 `STATEMENT_SEL` 单表读 `statements` 视图（无 JOIN）；实体维度经 `statement_*_rel` 反查。控制台工具：`console_add_prediction`（返回 statement_predict 行 id，可复用）、`console_update_status`（写 `statement_predict.status_code`）、`console_add_track`（写 `statement_rel`）、`blogger_trade`（写 `statement_trade`，**不收 op**）。
