...
46. 回复内容「原样照抄」——`reply_to` 是引文，不是摘要（2026-09-12 用户拍板）：
    ...
    - **采集侧连带**：`xueqiu-spyder` 必须保留 `回复@` / `//@` 引用结构（SKILL.md 已有此约束）；`post_history.raw_text` 是唯一留档（帖子集提炼后即删源文件），**导入时不得清洗引用块**。
...
48. 能用留档就别重采（2026-09-12 用户纠正）：
    ...
    - `post_history` 是**采集原文留档**（含 `form`、`raw_text`、`entities_json`、`post_id` 快照），存在的意义就是「不用再抓一次」。修数据先查留档：形态、回复内容（`//@` 段）、要素回溯都能在留档里解决（`backfill-reply-to.js`、`fix-form-from-archive.js`）。
