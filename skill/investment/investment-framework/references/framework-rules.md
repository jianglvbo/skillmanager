...
30. 博主档案「言论追踪」分类规范（2026-09-03 由四类 `kind` 改为内容五分）：按 `content_type` 分 **4 个 `###` 子表**（时间倒序，最近在上）——**研究 / 观点 / 心得总结 / 闲聊**；买卖记录走 #31，预测走「预测记录」表，均不在此内。各子表统一列：观点时间 | 发帖时间 | 内容 | 标的/行业 | 方向 | 观点（博主判断） | 信号 | 来源 | 原文链接。
    - **研究**：含数据/估值/行业结构的可复用分析。若已沉淀框架文件，观点列写 `见 [[分类/文件名]]`（画像记事实、wiki 记方法，不重复）。
    - **观点**：对个股/行业/市场/政策的判断，须填 `方向`（看多/看空/中性）；无信号则「信号」列填 `—`。
    - **心得总结**：投资心得、方法论、复盘框架。
