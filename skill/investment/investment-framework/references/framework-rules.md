...
31. 博主档案「个股买卖记录」规范：博主**明确买卖动作**（买入/加仓/减仓/卖出/清仓）**权威＝买卖帖言论行本身**：`post_trade`（经 MCP `blogger_trade` 写入），看板「个股买卖记录」段展示。**买卖帖只落这一张表**——原独立表 `blogger_trades` 已于 2026-09-12 并入并删除（一帖一表，见 #44）；**画像 md 不再镜像任何买卖记录**（画像 md 废弃，见 #36）。
...
35. 帖子「原文链接」统一规范（治本防悬空/丢失；2026-09-08 画像单轨化、2026-09-12 画像 md 废弃后只适用 DB）：`posts`（六表 UNION 视图）与其背后的六张帖子表 **一律必填 `source_url`**（服务端强校验），任意行原文链接禁止为空；买卖/预测的原文链接就在各自的言论行上（`post_trade` / `post_predict`），无需另表。画像 md 三表已退役（`assets/博主.md` 仅存历史参考）。
...
43. 批量数据修复脚本纪律（2026-09-11 教训：一个脚本写坏 50 行正文与回应）：言论表的批量修复**比新建更危险**——新建写错只影响新数据，修复写错会覆盖已经正确的历史行。凡对 `post_*` 做批量 UPDATE 的脚本，必须满足：
...
44. 数据库结构与命名规则（2026-09-11 拍板 + 2026-09-12 两次收口，投资看板 investment_kb）：
    ...
    - **废弃字段同样删除（2026-09-12 用户定）**：不再使用的列**删前把列值快照进 `backups/`，然后 `DROP COLUMN`**，不留 `*_del` 影子列。本批已删：六张帖子表的 `kind`（旧四类落位，`content_type` 已 100% 回填）与 `record_date`（历史记录日期，排序统一改用 `COALESCE(view_date, post_date)`）、`post_predict.verify_status_del`；随之失效的 dict 枚举 `coarse_status`/`track_direction`/`file_type`/`file_status` 一并清除（16 行）。视图 `posts` 同步收窄为 40 列。
