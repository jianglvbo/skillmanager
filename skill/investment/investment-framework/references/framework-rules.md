...
38. vault 链接渲染与主题归类规范（2026-09-03 用户提出）：
    ...
    - **旧主题维（`prediction_tracks`）迁移已收官（2026-09-07）**：803/803 行全部标 `migrated_to_statement_id` 迁回 `blogger_statements` 单轨（残余 123 条无主行经用户确认删除，2026-09-06），控制台不再有「待清理 / 待复核归零下架」语义。**新增言论一律走 `blogger_statement`（挂 `subject_id`），不得写本表**；本表仅保留"对某条预测的后续增强/反驳"本职用途（`console_add_track`），此类行出现在看板「预测跟踪」分组属正常展示。
    - **legacy 归因证据链（历史存档，仅供审计查询，不再有新迁移场景）**：① content 内 `[[博主/{X}/…]]` wikilink 且 X 已建档；② 同一段文字已存在于某博主画像言论表 → **合并指向既有行**（绝不新建第二份）；③ 采集批次名 `雪球采集-{博主昵称}-{日期}` 只命中一位已建档博主即为确定归属。归一化全文相等（或长文本前缀包含）、去空白与中英标点后比较、博主唯一才归，多博主同文不猜。
