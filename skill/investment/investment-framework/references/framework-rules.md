...
61. 行业归类单一真相源＝申万 `industry_sw`、挂到最细级；泛称先查询再落、境外无对应不强塞（2026-09-19 用户拍板）：
    ...
    - **挂到最细的准确级**：能落到三级（最底）就落三级，落不了退二级，再退一级。纯主业公司下钻到三级——例：天齐锂业→`有色金属>能源金属>锂`(L3)、寒武纪→`电子>半导体>数字芯片设计`(L3)、泸州老窖/茅台→`食品饮料>白酒Ⅱ>白酒`(L3)、盛达资源→`贵金属>白银`(L3)、招商轮船→`航运`(L3)。**多元化公司取覆盖其主业的准确级、别为凑三级挂错细分**——海尔智家→`白色家电`(L2，非"空调")、广汽集团→`乘用车`(L2)、三峡能源（风+光+水）→`电力`(L2)。**补挂前先查该股 `stock_industry_rel` 是否已有行业**：已有同级或更细的就别叠更粗的（会稽山已有`其他酒类(L3)`就不要再加`非白酒(L2)`），只在完全没有时按最细准确级补一条。
