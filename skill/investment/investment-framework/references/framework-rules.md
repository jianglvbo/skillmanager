...
60. 雪球行情标签「名称(代码)」＝确定个股，自动建档关联、绝不再当「解析不出」上报（2026-09-19 用户拍板，内化自待决策 #28 三峡能源 / #29 广汽集团）：
    - **用户原话**：「是个股啊，并且他后面还有括号带有代码呢，应该一眼能看出来这个是个股并且是港股」「这个代码一眼就能看出来是 A 股」。
    - **形态来源**：博主在帖子里链接个股时，雪球一律渲染成 `名称(代码)`——如 `三峡能源(SH600905)`、`广汽集团(02238)`、`信邦控股(01571)`、`海尔智家(600690/06690)`（A+H 双码）。括号前的中文名就是标准个股名，括号里的代码自带市场信息。
    ...
    - **硬要求（服务端）**：`linkSubjectsByText` 先摘出 `名称(代码)` 行情标签——已建档就按名挂，**没建档就按代码自动 `subjectOf(stock,…)` 建档再挂**，并把动作回写进 warnings（`已按代码自动建档个股「X(码)」并关联`）；标签从文本挖掉后再走纯名称拆段，避免 A+H 的 `/` 被分隔符拆坏。**纯汉字名（无代码）仍需在 `stock` 表或别名里命中才挂**，命不中才进待决策（如 #25 招商轮船 / #26 奇富科技 无代码 → 提炼侧按真名 `console_ensure_subject` 补建档，不靠正则猜）。
    ...
    - **自动建档只建了"个股主体"、不带行业**：`linkSubjectsByText` 按代码建档时 `industryName=null`，会造出"无行业个股"，违反"个股尽量挂行业"。**故 #60 建完个股后必须立刻按 #61 补挂最细级申万行业**（`industry_sw_list` 查询→ `stock_industry_rel`）；境外无申万主体的按 #61 允许留 0 行业，但要在汇报里说明"已建档·无申万行业"。绝不允许 #60 攒出一堆无行业个股。

61. 行业归类单一真相源＝申万 `industry_sw`、挂到最细级；泛称先查询再落、境外无对应不强塞（2026-09-19 用户拍板）：
    - **单一真相源**：挂行业 / 建行业实体 / 言论挂 `statement_industry_rel` 一律以 MySQL `industry_sw`（申万 2021：一级 31 / 二级 134 / 三级 335）为准，用 MCP `industry_sw_list` 取标准名；**指数**以 `index_catalog`、**市场**以封闭清单 `MARKET_CANON`（A股/港股/美股/韩股/汇率/虚拟货币/美债/国债/日债）。`tag-taxonomy.md` 第五节的申万名只是**标签展示词**——标签可含"AI与算力"等自定义一级，但**行业实体与关联必须命中 `industry_sw`**（`industryGate` 会拒非标名）。**指数 / 宽基 / 组合 / 泛称名一律不建 `statement_stock_rel`/`statement_industry_rel`**：解析遇指数名不得硬挂个股或申万行业，确属指数走 `market_index`、市场走封闭清单，否则 0 关联（#52，不报错、不补登、不新建非标行业）。
    - **挂到最细的准确级**：能落到三级（最底）就落三级，落不了退二级，再退一级。纯主业公司下钻到三级——例：天齐锂业→`有色金属>能源金属>锂`(L3)、寒武纪→`电子>半导体>数字芯片设计`(L3)、泸州老窖/茅台→`食品饮料>白酒Ⅱ>白酒`(L3)、盛达资源→`贵金属>白银`(L3)、招商轮船→`航运`(L3)。**多元化公司取覆盖其主业的准确级、别为凑三级挂错细分**——海尔智家→`白色家电`(L2，非"空调")、广汽集团→`乘用车`(L2)、三峡能源（风+光+水）→`电力`(L2)。
