...
57. 指数 / 宽基 / 组合 / 泛称不作为个股或行业实体关联（2026-09-16 用户拍板，内化自待决策 #18）：
    - **规则**：「沪深300」「中证500」等指数名，以及组合、代号、泛称，**不建 `statement_stock_rel` / `statement_industry_rel` 实体关联**；内容确属指数/市场维度的走 market/index 维度，或干脆 0 关联（见 #52「无关联不是缺陷」）。
    - **解析行为**：标的名解析遇到指数名，**不得硬挂到某个个股或申万行业**；解析不到实体即丢弃该关联，不报错、不补登、不新建非标行业。
...
60. 雪球行情标签「名称(代码)」＝确定个股，自动建档关联、绝不再当「解析不出」上报（2026-09-19 用户拍板，内化自待决策 #28 三峡能源 / #29 广汽集团）：
    ...
    - **反面（仍不建实体的）**：板块/政策**泛称词**（农业、科技、消费、金融——见 #20 内化）不是个股，无代码的泛称**不得**塞进 `target`（承 #57）；只有雪球给定了 `名称(代码)` 这种**确定性行情标签**才触发自动建档。
