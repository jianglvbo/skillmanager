...
- **博主判断依赖看板博主表**：对照**看板 MySQL `bloggers` 表**（`GET /api/bloggers/live` 或 MCP `list_bloggers`）判断是否为已登记博主（`工作区/博主控制台.md` 已于 2026-09-07 退役删除，**不要再找该文件**）
...
- **绝不输出模板残留成分**（footnote-taxonomy 禁止行为 #5 · 2026-08-08 用户纠正）：模板为纯结构骨架（字段 + section 标题 + 表格表头，不含任何解释），产出文件不得含 `{...}` 花括号占位、`> 指引 blockquote`（如"原文链接必填真实雪球帖子 URL"、"示例：淘宝闪购单量追上美团"）、frontmatter 行内注释、全空表格占位行——轻创建博主目录时解释成分全部剔除（画像 md 已退役入 bloggers 表）；各 section 写作指引见 references/template-guide.md
...
- [ ] 是否未将未登记作者「补登」进看板 bloggers 表（仅对照判断，绝不自动新增；未登记作者其条目归「其他」层，见 framework-rules #12）？
