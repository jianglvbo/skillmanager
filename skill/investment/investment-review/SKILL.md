...
按 `investment-framework/references/review-rules.md`「复核建议处理（审查首步）」执行：`console_post_review(action=list, status=open)` 取全部未处理建议（返回含该言论当前 `contentType/stance/target/viewText` 上下文）→ 逐条按建议用 `blogger_post(action=update)` 修正归类 → `console_post_review(action=apply)` 置已处理；判断建议不成立则 `action=delete` 并在报告说明理由。**本步未处理完，不得进入 C/S 维度**；修正结果并入审查报告。
...
**C9**：输出内容审查报告
**C10**：言论追踪审计（**2026-09-12 起兼看「insight 帖具象化覆盖率」**——方法论沉淀是否只在嘴上说：该有框架条目的心得帖要有 `wiki_ref`（当前 4/135，缺口大）；不依赖已删除的 `transferable` 标签字段；**2026-09-11 扩一项原文留档覆盖率**：查 MySQL `post_history` 与 `posts` 的对照——近批采集是否都有留档、留档的 `form`/`raw_text_len` 是否缺失，用于发现「采集漏落库」；只报告不修改，缺留档不阻断审查，仅提示后续采集补上）——执行 `scripts/tracks_audit.py --vault <vault路径>`（可选 `--mysql`，凭据经环境变量 `DB_PASS` 注入、勿硬编码），核对 MySQL `posts` 数据质量（缺原文链接/direction 码值合法性/标的占位残留；买卖帖＝`post_trade` 言论行，已无独立买卖表）；画像 md 已彻底废弃（2026-09-12：服务端删除全部回写代码与 `/api/blogger/resync` 端点，存量文件由用户自行删除），不再做 md↔DB 对照；只报告不修改，问题并入内容审查报告
