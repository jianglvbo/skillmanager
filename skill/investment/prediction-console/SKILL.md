...
description: 维护投资看板预测控制台（MySQL 存储，方案 A：vault 不再存控制台 Markdown）。四控制台（个股/行业/指数/市场）的预测录入、状态验证、言论跟踪全部走 MCP console_* 工具落库投资看板。触发词：「预测控制台」「录入预测」「更新预测」「预测跟踪」「预测验证」。排除条件：纯聊天、非投资预测类内容。
version: 2.3.0
---
...
- 绝不把**不可证伪/纯叙事**的内容塞进**本控制台的验证闭环**（宏观叙事、情绪判断留在言论库或跳过）。**注意这与言论归类无关**（2026-09-14 用户放宽）：言论库的 `predict` 只要求「对未来的判断」，不可精确验证的预测**照样是 predict 言论**，只是不进这里的验证表——**不得因此把它改回 `view`**
...
### 第二步：判断 consoleType
- 有具体股票代码 → `stock`（个股）
- 行业（白酒、半导体、光伏设备等）→ `industry`，**必须先查 `industry_sw_list` 取申万标准名**（口径：申万 2021 版三级体系；不写完整路径，只写最下级；查不到＝标准表没这个行业，别硬建）
- 指数（沪深300、创业板、恒生科技、中证人工智能主题指数、中证红利等）→ `index`，**先查 `index_catalog_list` 取目录名**（宽基/行业/主题/策略/风格/属性/债券/跨境八类；传别名如「创业板」也能命中目录名「创业板指」）
- 大盘走势（A股牛熊、港股、美股）→ `market`，但要**落到具体市场**（A股大盘→`A股`、美股大盘→`美股`）——`大盘` 本身不是市场主题，服务端会拒（2026-09-12 用户拍板：市场只有 A股/港股/美股/汇率/虚拟货币/美债/国债/日债这种）
...
- 行业：申万标准名的最下级（白酒 / 半导体 / 光伏设备），取自 `industry_sw_list`——**不写完整路径**（「白酒」而非「食品饮料/白酒」），**不写非标准/概念名**（AI与算力、锂、老登股 这类历史非标名已于 2026-09-15 全部迁移/清理完毕，现库内行业 100% 为申万标准名）
- 指数：指数目录规范名（沪深300 / 创业板指 / 中证人工智能主题指数），取自 `index_catalog_list`
...
- [ ] 行业名是否取自 `industry_sw_list`、指数名取自 `index_catalog_list`（未命中会被服务端拒绝并给候选）？
