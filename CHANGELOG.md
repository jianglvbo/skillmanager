...
### Changed
- **investment-framework** v2.13.0 → v2.14.0（模板纯结构化，用户 2026-08-08 理念确认）：
  - **9 个模板全部瘦身为纯结构骨架**：删除全部解释性成分（frontmatter 行内注释、花括号占位指引、blockquote 指引、表格示例行、空占位行），只保留字段骨架 + section 标题 + 表格表头
  - **新增 `references/template-guide.md`**：承接从模板剥离的全部 section 写作指引（各模板分节：用途/frontmatter 字段/section 填充要求），成为模板写作要求的唯一真相源
  - **footnote-taxonomy #5 语义更新**：模板 = 纯结构骨架，产出文件可保留空结构（空 section / 空表格表头）为合法形态；四类残留（花括号占位/blockquote 指引/frontmatter 注释/全空占位行）明确为违规
  - **verify-format.py 补花括号占位检测**（第 12 项，仅报告不自动修复）
- 配套 vault 批量清理：**308 个框架条目的 `delete:` 行尾注释泄漏全部剥离**（2026-08-05 批量补 delete 字段时带入模板注释的系统性污染），`--fix` 后复检 0 残留、字段结构完整

## 1.37.1 — 2026-08-08

### Changed
- **investment-framework** v2.12.0 → v2.13.0（审计修复）：
  - **模板 tags 示例对齐 tag-taxonomy**（5 处失同步修正）：分析档案模板删 `百润股份` 裸标签；宏观模板 `通用/宏观, 美联储, 货币政策` → `宏观/美联储, 宏观/货币政策`；方法论 `分析框架/框架类型/估值框架` → `分析框架/估值`（二级限：估值/基本面/商业模式/行业分析/宏观环境）；交易体系 `体系类型/仓位管理` → `交易体系/仓位管理`；投资心得 `心得主题/...` → `投资心得/买卖决策`；行业 `农林牧渔/养殖` → `农林牧渔/畜牧业`
  - **footnote-taxonomy 禁止行为 #5 扩充**：模板成分四类全禁（花括号指引/blockquote 指引/表格示例行/frontmatter 行内注释）+ 空表格占位行
  - **review-rules 结构审查 canonical 补 star/delete**：对齐 framework-rules #27（标准 8 字段）
  - **verify-format.py 补 3 项检测**：空表格占位行 / 来源 blockquote（`> 来源/原文链接/发布时间`）/ frontmatter 行尾注释泄漏，均可 `--fix`；**修复单问题文件漏报 bug**（`len(issues)>1` → `>0`，原逻辑单类问题文件全部静默漏报）
- 配套 vault 清理：华新建材(600801) 空表格占位行、老雷_复盘 末尾来源 blockquote 删除（原文链接已在 source 字段）
