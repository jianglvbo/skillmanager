...
--     ⑦ 来源路径字段退场（2026-09-12 同日）：六张言论表删 `src_rel`（帖子集不再落 vault，批次路径全是死链；
--        回指统一靠 `post_history_id`）；`refine_records.from_rel` 的帖子集值改写为 `post_history/{博主} {采集日}`（DB 引用，无 .md）。
