...
Usage:
    python audit.py <path> [--original <original_file>] [--auto-repair] [--author NAME]
...
from kit import FAMILY_BY_SUFFIX, repack, unzip_safely
from checks import DeckInspector, TrackedChangeAuditor, WordInspector
...
def main():
    ...
    if args.original:
        ...
        if original_file.suffix.lower() not in FAMILY_BY_SUFFIX:
            _fail(f"{original_file} must be one of: {', '.join(sorted(FAMILY_BY_SUFFIX))}")

    family = FAMILY_BY_SUFFIX.get((original_file or path).suffix.lower())
    if family is None:
        _fail(
            f"Cannot determine file type from {path}. Use --original or provide one of: {', '.join(sorted(FAMILY_BY_SUFFIX))}."
        )
    ...
    packed_file = None
    temp_dir_ctx = None
    if path.is_file() and path.suffix.lower() in FAMILY_BY_SUFFIX:
        packed_file = path
        temp_dir_ctx = tempfile.TemporaryDirectory()
        unpacked_dir = Path(temp_dir_ctx.name)
        try:
            with zipfile.ZipFile(path, "r") as zf:
                unzip_safely(zf, unpacked_dir)
    ...
    match family:
        case "docx":
            inspectors = [
                WordInspector(unpacked_dir, original_file, verbose=args.verbose),
            ]
            if args.author is not None:
                inspectors.append(
                    TrackedChangeAuditor(unpacked_dir, original_file, verbose=args.verbose)  
                )
        ...
        case "pptx":
            inspectors = [
                DeckInspector(unpacked_dir, original_file, verbose=args.verbose),
            ]
        case "xlsx":
            exts = ", ".join(k for k, v in sorted(FAMILY_BY_SUFFIX.items()) if v == "xlsx")
            print(
                f"No XSD schema validation is performed for xlsx-family files ({exts}). "
                "For formula-error checking, use toolkit/recalc.py instead."
            )
            sys.exit(0)
        case _:
    ...
    if args.auto_repair:
        total_repairs = sum(v.repair() for v in inspectors)
        if total_repairs:
    ...
    success = all([v.validate() for v in inspectors])
