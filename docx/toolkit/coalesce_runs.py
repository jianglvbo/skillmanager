...
Usage:
    python coalesce_runs.py unpacked/                  # after unzip, before editing
    python coalesce_runs.py document.docx              # rewrite in place
    python coalesce_runs.py document.docx -o out.docx
"""
...
from ooxml.kit import WS_CHARS, visible_text, repack, unzip_safely

WORDML_NS = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"


def coalesce_runs(input_dir: str) -> tuple[int, str]:
...
def _is_adjacent(elem1, elem2) -> bool:
    ...
    while node:
        ...
        if node.nodeType == node.ELEMENT_NODE:
            return False
        if node.nodeType == node.TEXT_NODE and node.data.strip(WS_CHARS):
            return False
...
def _rendered_text(elem) -> str:
    return visible_text(_element_text(elem), _has_preserve(elem))
...
def _consolidate_text_elements(run, tag: str):
    ...
    for i in range(len(t_elements) - 1, 0, -1):
        ...
        if _is_adjacent(prev, curr):
            ...
            if merged != merged.strip(WS_CHARS) or had_preserve:
...
def _coalesce_or_die(path: Path) -> str:
    _, msg = coalesce_runs(str(path))
