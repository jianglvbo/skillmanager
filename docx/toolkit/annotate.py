...
Usage:
    ...
    python annotate.py unpacked/ "Comment text"
    python annotate.py unpacked/ "Reply text" --parent 0

    # Against a .docx directly (extracts, writes satellite files, rezips)
    python annotate.py contract.docx "This cap is too low" -o annotated.docx
    python annotate.py contract.docx "Comment" --id 5      # explicit ID
...
from ooxml.kit import resolve_part, repack as _repack, unzip_safely as _unzip

TEMPLATE_DIR = Path(__file__).parent / "blueprints"
NS = {
...
def _ensure_comment_relationships(unpacked_dir: Path) -> None:
    ...
    for rel_type, target in _COMMENT_RELS:
        if resolve_part(target, "word/document.xml") in existing:
            continue
