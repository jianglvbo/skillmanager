...
**两个连带检查**：① 残留检测——看板已移除但 `博主/` 层仍有文件夹 → 报告并询问清理或迁移（防未登记博主悬空）；② info_cutoff 一致性——**只核对看板 `blogger.info_cutoff_datetime`**（规则 #36；画像 md 已于 2026-09-12 退役，不再有画像侧可对）。批量采集前想快速看**谁最该补抓**，在 investment-console 仓根跑 `node src/scripts/fetch-progress.js`（按 info_cutoff 陈旧度倒序、含每位存帖/待提炼，只读）——陈旧置顶即本轮优先级。
...
| **别名匹配**（2026-09-23） | 导入按 blogger 的 name+**alias** 匹配作者名——别名必须与雪球昵称**逐字一致**（教训：「大道无形我有型」≠ 库里「大道无形我有形」，一字之差致 250 帖被跳过）；发现新昵称先 `update_blogger` 补 alias 再重导（import 幂等） |
