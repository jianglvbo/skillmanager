...
Usage:
    python deck_clone.py unpacked/ slide2.xml                 # duplicate slide2
    python deck_clone.py unpacked/ slideLayout3.xml           # new slide from a layout
    python deck_clone.py unpacked/ slide2.xml --after slide2.xml
    python deck_clone.py deck.pptx slide2.xml                 # rewrite deck.pptx in place
    python deck_clone.py deck.pptx slide2.xml -o out.pptx
...
BLANK_SLIDE_XML = '''<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
SHARED_PART_KINDS = ("chart", "diagramData", "oleObject", "package")

NOTES_SLIDE_TYPE = re.compile(r"""Type=["'][^"']*/relationships/notesSlide["']""")
RELATIONSHIP_TAG = re.compile(r"<Relationship\b[^>]*?(?:/>|>.*?</Relationship\s*>)", re.DOTALL)

SLD_ID_MIN = 256
SLD_ID_MAX = 2147483647
...
def next_slide_index(slides_dir: Path) -> int:
...
def spawn_from_layout(unpacked_dir: Path, layout_file: str, after: str | None = None) -> str:
    slides_dir = unpacked_dir / "ppt" / "slides"
    rels_dir = slides_dir / "_rels"
    layout_path = unpacked_dir / "ppt" / "slideLayouts" / layout_file

    if not layout_path.exists():
        _abort(f"{layout_path} not found")

    next_num = next_slide_index(slides_dir)
    dest = f"slide{next_num}.xml"
    after_rid = _preflight_registration(unpacked_dir, after, dest)
    slides_dir.mkdir(parents=True, exist_ok=True)

    (slides_dir / dest).write_text(BLANK_SLIDE_XML, encoding="utf-8")
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/{layout_file}"/>
    ...
    _register_new_slide(unpacked_dir, dest, layout_file, after_rid)
    return dest


def copy_existing_slide(unpacked_dir: Path, source: str, after: str | None = None) -> str:
    slides_dir = unpacked_dir / "ppt" / "slides"
    rels_dir = slides_dir / "_rels"
    source_slide = slides_dir / source

    if not source_slide.exists():
        _abort(f"{source_slide} not found")
    ...
    if source_rels.exists():
        ...
        rels_content = RELATIONSHIP_TAG.sub(
            lambda m: "" if NOTES_SLIDE_TYPE.search(m.group(0)) else m.group(0),
            rels_content,
        )
        dest_rels.write_text(rels_content, encoding="utf-8")
        shared_parts = sorted({
            t for t in re.findall(r'Type="[^"]*/relationships/(\w+)"', rels_content)
            if t in SHARED_PART_KINDS
        })

    _register_new_slide(unpacked_dir, dest, source, after_rid)
    if shared_parts:
        print(
...
def _preflight_registration(unpacked_dir: Path, after: str | None, dest: str) -> str | None:
    pres_path = unpacked_dir / "ppt" / "presentation.xml"
    if not pres_path.exists():
        _abort(f"{pres_path} not found — is this an unpacked PPTX?")
    ...
    if not has_slot:
        _abort("presentation.xml has no <p:sldIdLst> (or <p:sldMasterIdLst> to anchor a new one)")
    ...
    if pres_rels.exists() and _slide_rel_id(
    ...
    if stale:
        _abort(
            ...
            f"run deck_prune.py first"
        )

    if not after:
        return None
    after_rid = _rid_of_slide(unpacked_dir, after)
    if not re.search(rf'<p:sldId\b[^>]*r:id="{re.escape(after_rid)}"[^>]*>', xml):
        _abort(f"{after} ({after_rid}) is not listed in <p:sldIdLst>")
    return after_rid


def _register_new_slide(unpacked_dir: Path, dest: str, source_desc: str, after_rid: str | None) -> None:
    _register_content_type(unpacked_dir, dest)
    rid = _register_presentation_rel(unpacked_dir, dest)
    slide_id = _next_slide_id(unpacked_dir)
    pos, total = _insert_sld_id(unpacked_dir, slide_id, rid, after_rid)
...
def _register_content_type(unpacked_dir: Path, dest: str) -> None:
...
def _register_presentation_rel(unpacked_dir: Path, dest: str) -> str:
    pres_rels_path = unpacked_dir / "ppt" / "_rels" / "presentation.xml.rels"
    pres_rels = pres_rels_path.read_text(encoding="utf-8")

    existing = _slide_rel_id(pres_rels, dest)
    if existing:
        return existing
...
def _slide_rel_id(pres_rels: str, slide_name: str) -> str | None:
...
def _next_slide_id(unpacked_dir: Path) -> int:
    ...
    candidate = max((i for i in used if i >= SLD_ID_MIN), default=SLD_ID_MIN - 1) + 1
    if candidate <= SLD_ID_MAX and candidate not in used:
        return candidate
    for i in range(SLD_ID_MIN, SLD_ID_MAX + 1):
        if i not in used:
            return i
    _abort("no slide id available in [256, 2147483647] — the deck is full")
...
def _insert_sld_id(
    ...
    if f'r:id="{rid}"' in xml:
        _abort(f"presentation.xml already references {rid}; refusing to add a duplicate")

    if after_rid:
        open_tag = re.search(rf'<p:sldId\b[^>]*r:id="{re.escape(after_rid)}"[^>]*>', xml)
        if not open_tag:
            _abort(f"{after_rid} is not listed in <p:sldIdLst>")
        end = open_tag.end()
        if not open_tag.group(0).endswith("/>"):
            close = xml.find("</p:sldId>", end)
            if close == -1:
                _abort(f"unclosed <p:sldId> for {after_rid} in presentation.xml")
...
def _rid_of_slide(unpacked_dir: Path, slide_name: str) -> str:
    pres_rels_path = unpacked_dir / "ppt" / "_rels" / "presentation.xml.rels"
    rid = _slide_rel_id(pres_rels_path.read_text(encoding="utf-8"), slide_name)
    if not rid:
        _abort(f"{slide_name} has no relationship in presentation.xml.rels")
    return rid


def clone_or_instantiate(unpacked_dir: Path, source: str, after: str | None = None) -> str:
    source_type, layout_file = classify_source(source)
    if source_type == "layout" and layout_file is not None:
        return spawn_from_layout(unpacked_dir, layout_file, after)
    return copy_existing_slide(unpacked_dir, source, after)


def clone_into_package(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with zipfile.ZipFile(package) as zf:
            unzip_guarded(zf, tmp_path)
        dest = clone_or_instantiate(tmp_path, source, after)
        repack(tmp_path, out)
...
def main() -> None:
    ...
    if target.is_dir():
        ...
        clone_or_instantiate(target, args.source, args.after)
    elif target.is_file() and target.suffix.lower() in (".pptx", ".potx"):
        try:
            clone_into_package(target, args.source, args.after, Path(args.output) if args.output else None)
        except (OSError, ValueError, zipfile.BadZipFile) as e:
            _abort(str(e))
    else:
        _abort(f"{target} is neither a directory nor a .pptx/.potx file")
