...
- 用户要做**行业横向对比**（涨跌排名 / 资金流入 / 领涨股 / 板块资金流向）
- 用户要看**融资融券 / 两融数据**（融资余额 + 融券余额）
- 用户要看**大宗交易**（成交价/量 + 买卖方营业部）
- 用户要看**股东户数变化**（筹码集中度）
- 用户要看**分红送转历史**（每股派息 + 送股 + 转增）
- 用户要做**股息率计算 / TTM 分红查询 / 高股息选股**（产品推荐 / 价值投资场景）
- 用户要看**指数/ETF行情**（上证指数 / 沪深300 / 创业板指 / ETF）
- 用户要看新闻资讯（个股新闻 / 财联社快讯 / 全球资讯）
- 用户要查公告（巨潮公告全文）
- 用户要做产业链调研 / 批量横向对比
- 关键词：估值、一致预期、机构预测、市盈率、PEG、市值、研报、产业链、行业研究、K线、盘口、公告、新闻、**强势股、题材、热点、概念归因、北向资金、沪股通、深股通、概念板块、资金流向、主力、龙虎榜、席位、营业部、全市场龙虎榜、净买入、解禁、限售、行业对比、行业轮动、融资融券、两融、大宗交易、股东户数、筹码集中、分红、派息、送股、股息率、指数、ETF**
...
### 3.7 行业板块排名 + 资金流（V3.0 改用东财 — 同花顺加了反爬401）

东财行业板块涨跌幅排名 + 板块级资金流向，一次调用覆盖行业轮动和资金面分析。

> **重要：** 当用户需要板块级资金流向分析时，必须使用 `industry_comparison()`——它已内置资金流字段，**禁止** Agent 自行编写 push2 底层接口调用代码。自行拼接接口参数极易出错（secid 格式、fields 编码、字段映射），且可能绕过 `em_get()` 防封限流。

```python
import requests

# push2 行业板块资金流字段映射（与个股资金流 eastmoney_fund_flow_minute 对齐）:
#   f62  = 主力净流入(元)        f66  = 超大单净流入(元)
#   f72  = 大单净流入(元)        f78  = 中单净流入(元)
#   f84  = 小单净流入(元)        f184 = 主力净占比(%)

def industry_comparison(top_n: int = 20) -> dict:
    """
    全行业涨跌幅排名 + 板块资金流向（东财行业板块，~100 个行业）。
    返回: {top: [...], bottom: [...], total: int}
    每条记录含涨跌、上涨/下跌家数、领涨股及板块级资金流数据。
    """
    ...
    def _safe_float(val, default="N/A"):
        """安全转浮点；None/空串/非数值 → default（不抛异常，保证批量可用）。"""
    ...
    for i, item in enumerate(items):
        rows.append({
            ...
            # 板块资金流（部分时段可能为空/延迟，标 N/A 而非报错）
            "main_net": _safe_float(item.get("f62")),          # 主力净流入(元)
            "super_net": _safe_float(item.get("f66")),         # 超大单净流入(元)
            "large_net": _safe_float(item.get("f72")),         # 大单净流入(元)
            "mid_net": _safe_float(item.get("f78")),           # 中单净流入(元)
            "small_net": _safe_float(item.get("f84")),         # 小单净流入(元)
            "main_ratio": _safe_float(item.get("f184")),       # 主力净占比(%)
        })
...
for r in data["top"][:10]:
    main_flow = f"{r['main_net']/1e4:.0f}万" if isinstance(r['main_net'], (int, float)) else "N/A"
    print(f"  {r['rank']}. {r['name']}: {r['change_pct']}% 涨{r['up_count']}跌{r['down_count']} 领涨{r['leader']} 主力净流入{main_flow}")
print("\nBOTTOM 5 跌幅:")
...
# 用法: 按主力净流入排序找资金驱动板块
fund_driven = sorted(
    [r for r in data.get("top", []) + data.get("bottom", []) if isinstance(r.get("main_net"), (int, float))],
    key=lambda r: r["main_net"], reverse=True
)
print("\n主力净流入 TOP 5 板块:")
for r in fund_driven[:5]:
    print(f"  {r['name']}: 主力净流入 {r['main_net']/1e4:.0f}万 涨幅{r['change_pct']}%")
```
...
> **数据源优先级（股息率计算场景）：**
> 当用户需要股息率（dividend yield）或 TTM 分红总额时，**必须优先使用 `dividend_history()`（东财分红页面，含完整历史每股派息）**。腾讯行情 API 仅含最近一次派息的有限字段，基于它计算股息率会导致数据不完整（仅覆盖最近一次派息），直接影响选股排序和推荐结论的准确性。
>
> 适用场景：product_recommendation（高股息产品筛选）、value_investment（价值投资股息率校验）、估值分析（股息率作为估值参考维度）。

```python
def dividend_history(code: str, page_size: int = 20) -> list[dict]:
    """
    分红送转历史 —— 股息率计算的首选数据源。
    数据来源: 东财 datacenter RPT_SHAREBONUS_DET（含完整历史每股派息/送股/转增）。
    返回: [{date, bonus_rmb(每股派息税前), transfer_ratio(转增比例), bonus_ratio(送股比例), plan(进度)}]
    """
    data = eastmoney_datacenter(
        "RPT_SHAREBONUS_DET",
        filter_str=f'(SECURITY_CODE="{code}")',
...
def get_dividend_yield_ttm(code: str, current_price: float, months: int = 12) -> dict:
    """
    TTM 股息率 —— 股息率计算的标准方法。
    计算窗口: 默认最近 12 个月（months=12），也可设为 4 获取最近 4 个季度派息。
    返回: {ttm_dividend_per_share, dividend_yield_pct, period_count, data_cutoff_date, note}
    data_cutoff_date 为分红记录中最晚除权日，用于标注数据时效性（年报/半年报
    集中披露后东财分红页面可能有数日更新延迟）。
    """
    from datetime import datetime, timedelta
    cutoff_date = datetime.now() - timedelta(days=months * 30)
    history = dividend_history(code, page_size=20)
    ttm_total = 0.0
    period_count = 0
    latest_ex_date = ""
    for d in history:
        ...
        try:
            ...
            continue
        if ex_dt >= cutoff_date:
            bonus = d.get("bonus_rmb", 0) or 0
            ttm_total += bonus
            period_count += 1
            if ex_date > latest_ex_date:
                latest_ex_date = ex_date
    if current_price <= 0:
        return {"ttm_dividend_per_share": ttm_total, "dividend_yield_pct": None,
                "period_count": period_count, "data_cutoff_date": latest_ex_date,
                "note": "股价<=0，无法计算股息率"}
    yield_pct = round(ttm_total / current_price * 100, 2)
    note = ""
    if period_count == 0:
        note = "最近12个月无分红记录，可能为成长股或数据延迟（年报/半年报披露后东财可能数日未更新）"
    elif latest_ex_date:
        # 如果最新除权日距当前超过 60 天，提示数据可能延迟
        try:
            latest_dt = datetime.strptime(latest_ex_date, "%Y-%m-%d")
            if (datetime.now() - latest_dt).days > 60:
                note = f"最新除权日 {latest_ex_date} 距当前已超60天，分红数据可能未反映最新派息方案；可交叉校验腾讯行情 API 的最近一次派息字段"
        except ValueError:
            pass
    return {
        "ttm_dividend_per_share": ttm_total,
        "dividend_yield_pct": yield_pct,
        "period_count": period_count,
        "data_cutoff_date": latest_ex_date,
        "note": note,
    }
...
# 用法: TTM 股息率（标准方法，适用于 product_recommendation / value_investment 场景）
from datetime import datetime
# 先获取当前股价
price = tencent_quote(["600519"]).get("600519", {}).get("price", 0)
ttm = get_dividend_yield_ttm("600519", current_price=price, months=12)
print(f"TTM 股息率: {ttm['dividend_yield_pct']}% (近12月派息 {ttm['period_count']} 次，每股合计 {ttm['ttm_dividend_per_share']} 元)")
if ttm['note']:
    print(f"  ⚠ {ttm['note']}")
```
...
# 12. 股息率（TTM，标准方法）
ttm = get_dividend_yield_ttm(code, current_price=q["price"], months=12)
print(f"TTM 股息率: {ttm['dividend_yield_pct']}% (近12月派息{ttm['period_count']}次)")
if ttm['note']:
    print(f"  ⚠ {ttm['note']}")
```
